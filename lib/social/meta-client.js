...
import { instagramPilotWriteGateDiagnostics } from './write-gates.js';
...
export async function diagnoseMetaConnection({ env = process.env, fetchImpl = fetch } = {}) {
  ...
  const writeGate = instagramPilotWriteGateDiagnostics(env);
  ...
  const result = {
    ...
    publishingEnabled: writeGate.enabled,
    writeGate,
