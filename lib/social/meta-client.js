...
export async function diagnoseMetaConnection({ env = process.env, fetchImpl = fetch } = {}) {
  ...
  try {
    ...
    const pages = await graphGet('PAGE_DISCOVERY', 'me/accounts', config.META_FACEBOOK_PAGE_ACCESS_TOKEN, {
      fields: 'id,name,tasks,instagram_business_account',
      appsecret_proof: proof
    }, fetchImpl);
    const page = Array.isArray(pages.data)
      ? pages.data.find(item => String(item?.id || '') === config.META_FACEBOOK_PAGE_ID) || {}
      : {};
    ...
    const facebookChecks = [
      ...
      check('PAGE_CONTENT_TASKS', contentTask, 'A pessoa/token deve possuir as tarefas CREATE_CONTENT e MANAGE ou controle total equivalente.'),
    ...
    result.instagram = {
      ...
      identity: instagramIdMatches ? { username: String(instagram.username || '') } : undefined,
