...
export async function listPackages() {
  ...
  const packages = await Promise.all(documents.map(async document => {
    ...
    const source = await resolveSource(value.sourceType, value.sourceId, value.language);
    const current = isSourceCurrent(value, source);
    return { ...value, sourceStale: !current, approvalValid: value.status === 'APPROVED' && current };
  }));
