...
export function isPackageVisibleInAdmin(value) {
  return value?.status !== 'CANCELED';
}

export async function listPackages() {
  const documents = await listDocuments(COLLECTION);
  const values = documents.map(documentData).filter(isPackageVisibleInAdmin);
  const packages = await Promise.all(values.map(async value => {
